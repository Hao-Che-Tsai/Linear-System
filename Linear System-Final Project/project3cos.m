...
xt=zeros(2,length(t));
xt(:,1)=[((8*T/3)-2*T*exp(-0.5*Delta/T)-(2*T/3)*exp(-1.5*Delta/T))*sin(2*pi*Delta)/pi+((-28*T/3)+14*T*exp(-0.5*Delta/T)-(14*T/3)*exp(-1.5*Delta/T))*sin(2*pi*Delta)/8*pi;
         ((-2*T/3)+T*exp(-0.5*Delta/T)-(T/3)*exp(-1.5*Delta/T))*sin(2*pi*Delta)/pi+((28*T/3)-7*T*exp(-0.5*Delta/T)-(7*T/3)*exp(-1.5*Delta/T))*sin(2*pi*Delta)/8*pi];

for j=1:length(t)-1
    xt(:,j+1)=Ad*xt(:,j)+Bd*u(:,j);
    yt(:,j+1)=Cd*xt(:,j);
end
